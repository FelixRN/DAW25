...
public class MainController {
	...
	public void saveData(List<Car> listForSave) throws IOException {
			    ...
			    writer.close();
			    TerminalUtils.output("Datos guardados en: " + file.getAbsolutePath());
	}


	public void loadData() throws IOException {
	    File fileLoad = new File("data.model"); // Usar la misma ruta que en saveData
	    try (Scanner sc = new Scanner(fileLoad)) {
	        model.deleteList();//Borrar la lista actual
	        
	        while (sc.hasNextLine()) {
	            String line = sc.nextLine();
	            if (line.trim().isEmpty()) continue; // Salta líneas vacías
	            
	            String[] parts = line.split("\\|"); // Usa | como separador
	            
	            if (parts.length >= 3) { // Asegúrate de que hay suficientes partes
	                // Índice 0 debe ser "Car", índice 1 es plate, índice 2 es model
	                Car newCar = new Car(parts[1].trim(), parts[2].trim());
	                
	                // Verifica si el coche ya existe antes de agregarlo
	                Car existingCar = model.findByPlate(parts[1].trim());
	                if (existingCar == null) {
	                    model.add(newCar);
	                } else {
	                    // El coche ya existe, podrías actualizarlo o ignorarlo
	                    TerminalUtils.output("Coche con matrícula " + parts[1].trim() + " ya existe, se ignorará.");
	                }
	            } else {
	                TerminalUtils.output("Formato incorrecto en línea: " + line);
	            }
	        }
	        TerminalUtils.output("Datos cargados correctamente.");
	    } catch (Exception e) {
	        TerminalUtils.output("Error al cargar los archivos: " + e.getMessage());
	        e.printStackTrace();
	    }
	}
